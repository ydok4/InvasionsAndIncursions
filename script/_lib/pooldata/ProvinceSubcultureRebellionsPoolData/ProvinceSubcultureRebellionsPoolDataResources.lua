function GetProvinceSubcultureRebellionsPoolDataResources()
    return {
        wh2_main_aghol_wastelands = {

        },
        wh2_main_albion = {

        },
        wh2_main_ash_river = {

        },
        wh2_main_atalan_mountains = {

        },
        wh2_main_avelorn = {

        },
        wh2_main_blackspine_mountains = {

        },
        wh2_main_caledor = {

        },
        wh2_main_charnel_valley = {

        },
        wh2_main_chrace = {

        },
        wh2_main_coast_of_araby = {

        },
        wh2_main_cothique = {

        },
        wh2_main_crater_of_the_walking_dead = {

        },
        wh2_main_deadwood = {

        },
        wh2_main_devils_backbone = {

        },
        wh2_main_doom_glades = {

        },
        wh2_main_eagle_gate = {

        },
        wh2_main_eataine = {

        },
        wh2_main_ellyrion = {

        },
        wh2_main_galleons_graveyard = {

        },
        wh2_main_great_desert_of_araby = {

        },
        wh2_main_great_mortis_delta = {

        },
        wh2_main_griffon_gate = {

        },
        wh2_main_headhunters_jungle = {

        },
        wh2_main_heart_of_the_jungle = {

        },
        wh2_main_hell_pit = {

        },
        wh2_main_huahuan_desert = {

        },
        wh2_main_iron_mountains = {

        },
        wh2_main_ironfrost_glacier = {

        },
        wh2_main_isthmus_of_lustria = {

        },
        wh2_main_jungles_of_green_mists = {

        },
        wh2_main_kingdom_of_beasts = {

        },
        wh2_main_land_of_assassins = {

        },
        wh2_main_land_of_the_dead = {

        },
        wh2_main_land_of_the_dervishes = {

        },
        wh2_main_nagarythe = {

        },
        wh2_main_northern_great_jungle = {

        },
        wh2_main_northern_jungle_of_pahualaxa = {

        },
        wh2_main_obsidian_peaks = {

        },
        wh2_main_phoenix_gate = {

        },
        wh2_main_saphery = {

        },
        wh2_main_sartosa = {

        },
        wh2_main_shifting_sands = {

        },
        wh2_main_skavenblight = {

        },
        wh2_main_southern_great_jungle = {

        },
        wh2_main_southern_jungle_of_pahualaxa = {

        },
        wh2_main_southlands_jungle = {

        },
        wh2_main_southlands_worlds_edge_mountains = {

        },
        wh2_main_spine_of_sotek = {

        },
        wh2_main_the_black_coast = {

        },
        wh2_main_the_black_flood = {

        },
        wh2_main_the_broken_land = {

        },
        wh2_main_the_chill_road = {

        },
        wh2_main_the_clawed_coast = {

        },
        wh2_main_the_creeping_jungle = {

        },
        wh2_main_the_road_of_skulls = {

        },
        wh2_main_tiranoc = {

        },
        wh2_main_titan_peaks = {

        },
        wh2_main_unicorn_gate = {

        },
        wh2_main_vampire_coast = {

        },
        wh2_main_volcanic_islands = {

        },
        wh2_main_vor_aghol_wastelands = {

        },
        wh2_main_vor_albion = {

        },
        wh2_main_vor_ash_river = {

        },
        wh2_main_vor_ashen_coast = {

        },
        wh2_main_vor_avelorn = {

        },
        wh2_main_vor_caledor = {

        },
        wh2_main_vor_central_spine_of_sotek = {

        },
        wh2_main_vor_chrace = {

        },
        wh2_main_vor_coast_of_araby = {

        },
        wh2_main_vor_coast_of_squalls = {

        },
        wh2_main_vor_coasts_of_the_lustrian_gulf = {

        },
        wh2_main_vor_cobra_pass = {

        },
        wh2_main_vor_copper_desert = {

        },
        wh2_main_vor_cothique = {

        },
        wh2_main_vor_culchan_plains = {

        },
        wh2_main_vor_deadwood = {

        },
        wh2_main_vor_eagle_gate = {

        },
        wh2_main_vor_eataine = {

        },
        wh2_main_vor_ellyrion = {

        },
        wh2_main_vor_forest_of_the_viper = {

        },
        wh2_main_vor_fortress_of_dawn = {

        },
        wh2_main_vor_galleons_graveyard = {

        },
        wh2_main_vor_great_mortis_delta = {

        },
        wh2_main_vor_grey_guardians = {

        },
        wh2_main_vor_griffon_gate = {

        },
        wh2_main_vor_headhunters_jungle = {

        },
        wh2_main_vor_heart_of_the_jungle = {

        },
        wh2_main_vor_iron_mountains = {

        },
        wh2_main_vor_iron_peaks = {

        },
        wh2_main_vor_ironfrost_glacier = {

        },
        wh2_main_vor_isthmus_of_lustria = {

        },
        wh2_main_vor_jungle_of_pahualaxa = {

        },
        wh2_main_vor_jungles_of_green_mist = {

        },
        wh2_main_vor_kingdom_of_beasts = {

        },
        wh2_main_vor_land_of_assassins = {

        },
        wh2_main_vor_land_of_the_dead = {

        },
        wh2_main_vor_land_of_the_dervishes = {

        },
        wh2_main_vor_mosquito_swamps = {

        },
        wh2_main_vor_nagarythe = {

        },
        wh2_main_vor_naggarond = {

        },
        wh2_main_vor_northern_great_jungle = {

        },
        wh2_main_vor_northern_red_rivers = {

        },
        wh2_main_vor_northern_spine_of_sotek = {

        },
        wh2_main_vor_northern_yvresse = {

        },
        wh2_main_vor_obsidian_peaks = {

        },
        wh2_main_vor_phoenix_gate = {

        },
        wh2_main_vor_plain_of_spiders = {

        },
        wh2_main_vor_red_desert = {

        },
        wh2_main_vor_river_qurveza = {

        },
        wh2_main_vor_saphery = {

        },
        wh2_main_vor_sartosa = {

        },
        wh2_main_vor_scorpion_coast = {

        },
        wh2_main_vor_sea_of_dread = {

        },
        wh2_main_vor_settlers_coast = {

        },
        wh2_main_vor_shadow_wood = {

        },
        wh2_main_vor_shifting_mangrove_coastline = {

        },
        wh2_main_vor_shifting_sands = {

        },
        wh2_main_vor_southern_badlands = {

        },
        wh2_main_vor_southern_great_jungle = {

        },
        wh2_main_vor_southern_jungles = {

        },
        wh2_main_vor_southern_red_rivers = {

        },
        wh2_main_vor_southern_spine_of_sotek = {

        },
        wh2_main_vor_southern_yvresse = {

        },
        wh2_main_vor_southlands_world_edge_mountains = {

        },
        wh2_main_vor_straits_of_lothern = {

        },
        wh2_main_vor_the_black_coast = {

        },
        wh2_main_vor_the_black_flood = {

        },
        wh2_main_vor_the_broken_land = {

        },
        wh2_main_vor_the_capes = {

        },
        wh2_main_vor_the_chill_road = {

        },
        wh2_main_vor_the_clawed_coast = {

        },
        wh2_main_vor_the_creeping_jungle = {

        },
        wh2_main_vor_the_dragon_isles = {

        },
        wh2_main_vor_the_forbidden_jungle = {

        },
        wh2_main_vor_the_great_desert = {

        },
        wh2_main_vor_the_jungles_of_the_gods = {

        },
        wh2_main_vor_the_lost_valleys = {

        },
        wh2_main_vor_the_red_rivers = {

        },
        wh2_main_vor_the_road_of_skulls = {

        },
        wh2_main_vor_the_turtle_isles = {

        },
        wh2_main_vor_the_vampire_coast = {

        },
        wh2_main_vor_tiranoc = {

        },
        wh2_main_vor_unicorn_gate = {

        },
        wh2_main_vor_volcanic_islands = {

        },
        wh2_main_vor_western_jungles = {

        },
        wh2_main_western_jungles = {

        },
        wh2_main_yvresse = {

        },
        wh_main_argwylon = {

        },
        wh_main_averland = {

        },
        wh_main_bastonne_et_montfort = {

        },
        wh_main_black_mountains = {

        },
        wh_main_blightwater = {

        },
        wh_main_blood_river_valley = {

        },
        wh_main_bordeleaux_et_aquitaine = {
            IsAdjacentToSea = true,
            RebelSubcultures = {
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh2_main_sc_def_dark_elves = {
                    Weighting = 1,
                    ArmyArchetypes = {
                        Corsairs = {
                            Weighting = 3,
                        },
                    },
                },
            },
        },
        wh_main_carcassone_et_brionne = {
            IsAdjacentToSea = true,
            RebelSubcultures = {
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh2_main_sc_def_dark_elves = {
                    Weighting = 1,
                    ArmyArchetypes = {
                        Corsairs = {
                            Weighting = 3,
                        },
                    },
                },
            },
        },
        wh_main_couronne_et_languille = {
            IsAdjacentToSea = true,
            RebelSubcultures = {
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh2_main_sc_def_dark_elves = {
                    Weighting = 100,
                    ArmyArchetypes = {
                        Corsairs = {
                            Weighting = 3,
                        },
                    },
                },
            },
        },
        wh_main_death_pass = {

        },
        wh_main_desolation_of_nagash = {

        },
        wh_main_eastern_badlands = {

        },
        wh_main_eastern_border_princes = {

        },
        wh_main_eastern_oblast = {

        },
        wh_main_eastern_sylvania = {

        },
        wh_main_estalia = {

        },
        wh_main_forest_of_arden = {

        },
        wh_main_gianthome_mountains = {

        },
        wh_main_goromadny_mountains = {

        },
        wh_main_helspire_mountains = {

        },
        wh_main_hochland = {

        },
        wh_main_ice_tooth_mountains = {

        },
        wh_main_lyonesse = {
            IsAdjacentToSea = true,
            RebelSubcultures = {
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh2_main_sc_def_dark_elves = {
                    Weighting = 1,
                    ArmyArchetypes = {
                        Corsairs = {
                            Weighting = 3,
                        },
                    },
                },
            },
        },
        wh_main_massif_orcal = {

        },
        wh_main_middenland = {

        },
        wh_main_mountains_of_hel = {

        },
        wh_main_mountains_of_naglfari = {

        },
        wh_main_nordland = {

        },
        wh_main_northern_grey_mountains = {

        },
        wh_main_northern_oblast = {

        },
        wh_main_northern_worlds_edge_mountains = {

        },
        wh_main_ostermark = {

        },
        wh_main_ostland = {

        },
        wh_main_parravon_et_quenelles = {

        },
        wh_main_peak_pass = {

        },
        wh_main_reikland = {
            IsAdjacentToSea = false,
            RebelSubcultures = {
                wh_dlc03_sc_bst_beastmen = {
                    Weighting = 3,
                },
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        NightGoblinWarboss = {
                            Weighting = 2,
                        },
                        GoblinGreatShaman = {
                            Weighting = 2,
                        },
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh_main_sc_vmp_vampire_counts = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        MasterNecromancer = {
                            Weighting = 3,
                        },
                        StrigoiGhoulKing = {
                            Weighting = 1,
                        },
                        VampireLord = {
                            Weighting = 2,
                        },
                    },
                },
            },
        },
        wh_main_rib_peaks = {

        },
        wh_main_southern_badlands = {

        },
        wh_main_southern_grey_mountains = {

        },
        wh_main_southern_oblast = {

        },
        wh_main_stirland = {

        },
        wh_main_talabecland = {

        },
        wh_main_talsyn = {

        },
        wh_main_the_silver_road = {

        },
        wh_main_the_vaults = {

        },
        wh_main_the_wasteland = {
            IsAdjacentToSea = true,
            RebelSubcultures = {
                wh_main_sc_grn_greenskins = {
                    Weighting = 3,
                    ArmyArchetypes = {
                        OrcWarboss = {
                            Weighting = 1,
                        },
                    },
                },
                wh2_main_sc_def_dark_elves = {
                    Weighting = 1,
                    ArmyArchetypes = {
                        Corsairs = {
                            Weighting = 3,
                        },
                    },
                },
            },
        },
        wh_main_tilea = {

        },
        wh_main_torgovann = {

        },
        wh_main_troll_country = {

        },
        wh_main_trollheim_mountains = {

        },
        wh_main_vanaheim_mountains = {

        },
        wh_main_western_badlands = {

        },
        wh_main_western_border_princes = {

        },
        wh_main_western_sylvania = {

        },
        wh_main_wissenland = {

        },
        wh_main_wydrioth = {

        },
        wh_main_yn_edri_eternos = {

        },
        wh_main_zhufbar = {

        },
    };
end